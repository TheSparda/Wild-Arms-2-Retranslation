...
Parses translation/insert/*_FINAL.txt (4-column JP/LIT/EN/RE boxes) + the chapter map, and emits a single
self-contained interactive HTML file (wiki/index.html). Re-run after each mission to auto-update.

Usage:  python3 tools/build_wiki.py
"""
...
INSERT = os.path.join(ROOT, 'translation/insert')
GS_DIR = os.path.join(ROOT, 'data/script')
...
# area code -> data/script section id that contains it (for placeholder transcript fallback)
AREA_SECTION = {}
...
def gamescript_html(area_code):
    ...
    Prefers the per-location file (data/script/loc/<AREA>.txt) — split on the transcript's own
...
# area code -> data/script section id(s) that contain it (a section can list several areas)
_AREA_SIDS = {}
for _sid, _codes in _SECTION_AREAS.items():
    for _c in _codes:
...
def build():
    ...
    try:
        ...
        _dbrows = []
    # honest box-level coverage over the CORRECT box set (data/script/box_coverage.json),
    # generated by tools/box_coverage.py. The DB 'slot' counts below measure only the
    ...
    <h3>{esc(s['label'])} <span class="subm">block {esc(s['block'])} · {s['nboxes']} slots · <code>translation/insert/{esc(s['file'])}</code></span></h3>
    {boxes_html}
  </div>''')
  ...
  {gs_block}
            ...
            for s in scs:
                ...
                body.append(f'''  <div class="subscene">
    <h3>{esc(s['subtitle'])} <span class="subm">block {esc(s['block'])} · {s['nboxes']} slots · <code>translation/insert/{esc(s['file'])}</code></span></h3>
...
<main id="main">
  ...
  {body}
  <footer>
    Generated by <code>tools/build_wiki.py</code> on {now}. Re-run after each mission to update.<br>
...
function buildFeedback(s) {{
  ...
  keys.forEach(function(k) {{
    ...
    out += 'FILE: translation/insert/' + parts[0] + '\\n';
