...
Reads the master DB (data/script/wa2_db.json). Area->block mapping derived from build_db.SCENES +
FIRSTPASS. A block can hold >1 area, so slots are reported per BLOCK the area occupies (with a note).

  python3 tools/area_todo.py                 # summary: every area, translated vs untranslated slots
  python3 tools/area_todo.py <AREA>          # list the untranslated US# slots (with EN) for one area
  python3 tools/area_todo.py <AREA> --clean  # only slots whose decoded JP is clean (ready to translate)
"""
import sys, os, json
...
DBJ = os.path.join(ROOT, 'data/script', 'wa2_db.json')
...
def emit_json():
    """Write data/script/area_todo.json: area -> {done, todo, todo_clean} for the wiki to read."""
    db, byblk = load(); ab = area_blocks()
    out = {}
    for area in ab:
        blks, rows = area_rows(area, byblk, ab)
    ...
    p = os.path.join(ROOT, 'data/script', 'area_todo.json')
