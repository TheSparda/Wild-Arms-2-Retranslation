...
AL = json.load(open('data/jp_us_alignment.json'))
us2jp = {}
...
if __name__=='__main__':
    if len(sys.argv)>=3:
        ...
        import os
        os.makedirs('translation/insert',exist_ok=True)
        UBLK=90112
        def blk(i): return UM[i]//UBLK
        cur=blk(0); start=0; nf=0
        for i in range(1,len(UM)+1):
            if i==len(UM) or blk(i)!=cur:
                build(start,i,f'translation/insert/blk{cur:03d}.txt',f'US block {cur} (US#{start}-{i-1})'); nf+=1
                if i<len(UM): cur=blk(i); start=i
        print(f'wrote {nf} insertion block files to translation/insert/')
