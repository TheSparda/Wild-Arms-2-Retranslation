#!/usr/bin/env python3
"""
Parse the Unos Hambalos GameFAQs game-script transcript (translation/dumps_en/game script.rtf)
into a structured, per-section, speaker-tagged script — the MASTER COVERAGE ANCHOR.

Output:
  data/script/manifest.json         — sections, per-section box counts + speaker rosters
  data/script/[X.YY].txt            — one flat file per guide section (speaker-tagged boxes)
...
Usage: python3 tools/parse_gamescript.py
"""
...
RTF  = os.path.join(ROOT, 'translation/dumps_en', 'game script.rtf')
OUT  = os.path.join(ROOT, 'data/script')
...
def main():
    ...
    text = load_text()
    sections = parse(text)
    manifest = {'source':'translation/dumps_en/game script.rtf','sections':[]}
    for s in sections:
        ...
        manifest['sections'].append({
            ...
            'roster':roster[:12],'file':'data/script/'+s['id']+'.txt'})
    ...
    print(f"parsed {len(sections)} sections, {tot} dialogue boxes -> data/script/")
    ...
    print(f"\nper-LOCATION: {len(loc_manifest)} areas, {ltot} dialogue boxes -> data/script/loc/")
