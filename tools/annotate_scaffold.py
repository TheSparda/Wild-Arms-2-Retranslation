"""Annotate every translation/blocks/blkNNN.txt box with a per-box confidence + flags header line,
...
files=sorted(f for f in os.listdir('translation/blocks') if re.match(r'blk\d+\.txt$',f))
total_ok=total_fair=total_shaky=total_jponly=0
for fn in files:
    path='translation/blocks/'+fn
